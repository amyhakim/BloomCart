    ...
    window.chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        resolve(
          response ?? {
            ok: false,
            error: "The BloomCart extension did not respond.",
          },
        );
      },
    );
...
function cartCaptureToProducts(cart: CartCapture): Product[] {
  return cart.products.slice(0, MAX_EXTENSION_PRODUCTS).map((item, index) => {
    ...
    const hash = getStableHash(
      `${cart.supportedSite}|${name}|${price}|${item.link ?? index}`,
    );
...
function App() {
  ...
  const syncExtensionCart = useCallback(async (manual = false) => {
    if (manual) setNotification("Checking the BloomCart extension…");
    const response = await getExtensionCart();

    if (!response.ok) {
      if (manual || !lastSignature.current) {
        setNotification(
          `Extension not connected: ${response.error ?? "unknown error"}`,
        );
      }
      return;
    }
    if (!response.cart) {
      if (manual || !lastSignature.current)
        setNotification("Waiting for a supported cart page.");
      return;
    }

    const signature = JSON.stringify({
      site: response.cart.supportedSite,
      url: response.cart.sourceUrl,
      products: response.cart.products.slice(0, MAX_EXTENSION_PRODUCTS),
    });
    if (!manual && signature === lastSignature.current) return;
    lastSignature.current = signature;

    const captured = cartCaptureToProducts(response.cart);
    setProducts(captured);
    setView("recently");
    setNotification(
      captured.length
        ? `${captured.length} item${captured.length === 1 ? "" : "s"} archived from ${response.cart.supportedSite}.`
        : `No products found in the ${response.cart.supportedSite} cart.`,
    );
    setIsAnalyzing(true);
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => void syncExtensionCart(), 0);
    const interval = window.setInterval(
      () => void syncExtensionCart(),
    ...
    <div className="bc-shell">
      ...
      <div className="bc-actions">
        <button
          ...
          onClick={() => void syncExtensionCart(true)}
        >
          Refresh Cart
        </button>
        <button
